...
cv::Vec2f SegmentClassify::points_to_line(cv::Vec4f points) {
    auto x1 = points[0];
    auto y1 = points[1];
    auto x2 = points[2];
    auto y2 = points[3];
 ...
 */
cv::Mat SegmentClassify::detect_edges(cv::Mat& input) {
...
cv::Mat SegmentClassify::detect_edges2(cv::Mat& input) {
    cv::Mat gray, blurred, detect, hough1, hough2;
    hough1 = input.clone();
    hough2 = input.clone();
...
cv::Mat SegmentClassify::detect_edges3(cv::Mat& input) {
//    cv::Mat gray, blurred, detect, hough1, hough2;
//    hough2 = input.clone();
//
//    cv::cvtColor(input, gray, cv::COLOR_BGR2GRAY);
