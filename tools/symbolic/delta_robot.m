...
% Kinematics
xc11 = [0; lc1 * cos(phi1); lc1 * sin(phi1)]; % Position of Center of Mass of Link 1 of Arm 1 in Cartesian Coordintes
xc11dot = diff(xc11, phi1) * phi1dot; % Velocity of Center of Mass of Link 1 of Arm 1 in Cartesian Coordinates
xc12 = [lc2 * sin(phi2) * sin(phi3); l1 * cos(phi1) + lc2 * cos(phi2); l1 * sin(phi1) + lc2 * sin(phi2) * cos(phi3)]; % Position of Center of Mass of Link 2 of Arm 1 in Cartesian Coordinates
...
xc13 = [l2 * sin(phi2) * sin(phi3); l1 * cos(phi1) + l2 * cos(phi2); l1 * sin(phi1) + l2 * sin(phi2) * cos(phi3)]; % Position of Center of Mass of End-Effector of Arm 1 in Cartesian Coordinates
...
% Kinematics
xc21 = [0; lc1 * cos(phi1); lc1 * sin(phi1)];
xc21dot = diff(xc21, phi1) * phi1dot;
xc22 = [lc2 * sin(phi2) * sin(phi3); l1 * cos(phi1) + lc2 * cos(phi2); l1 * sin(phi1) + lc2 * sin(phi2) * cos(phi3)];
xc22dot = diff(xc22, phi1) * phi1dot + diff(xc22, phi2) * phi2dot + diff(xc22, phi3) * phi3dot;
xc23 = [l2 * sin(phi2) * sin(phi3); l1 * cos(phi1) + l2 * cos(phi2); l1 * sin(phi1) + l2 * sin(phi2) * cos(phi3)];
...
% Kinematics
xc31 = [0; lc1 * cos(phi1); lc1 * sin(phi1)];
xc31dot = diff(xc31, phi1) * phi1dot;
xc32 = [lc2 * sin(phi2) * sin(phi3); l1 * cos(phi1) + lc2 * cos(phi2); l1 * sin(phi1) + lc2 * sin(phi2) * cos(phi3)];
xc32dot = diff(xc32, phi1) * phi1dot + diff(xc32, phi2) * phi2dot + diff(xc32, phi3) * phi3dot;
xc33 = [l2 * sin(phi2) * sin(phi3); l1 * cos(phi1) + l2 * cos(phi2); l1 * sin(phi1) + l2 * sin(phi2) * cos(phi3)];
...
[c1, t1] = coeffs(-b(1), qdot);
[c2, t2] = coeffs(-b(2), qdot);
[c3, t3] = coeffs(-b(3), qdot);
[c4, t4] = coeffs(-b(4), qdot);
[c5, t5] = coeffs(-b(5), qdot);
[c6, t6] = coeffs(-b(6), qdot);
[c7, t7] = coeffs(-b(7), qdot);
[c8, t8] = coeffs(-b(8), qdot);
[c9, t9] = coeffs(-b(9), qdot);
G_minus_lambdaH = [
    t1(end) * c1(end)';
    t2(end) * c2(end)';
    t3(end) * c3(end)';
...
C = [
    t1(1:end-1) * c1(1:end-1)';
    t2(1:end-1) * c2(1:end-1)';
    t3(1:end-1) * c3(1:end-1)';
    t4(1:end-1) * c4(1:end-1)';
    t5(1:end-1) * c5(1:end-1)';
    t6(1:end-1) * c6(1:end-1)';
    t7(1:end-1) * c7(1:end-1)';
    t8(1:end-1) * c8(1:end-1)';
    t9(1:end-1) * c9(1:end-1)'
];

simplify(C + G_minus_lambdaH + b); % make sure this is 0

B = [1;0;0;1;0;0;1;0;0];
B = diag(B);
G = G_minus_lambdaH + (lambda' * H)';
